...
source "${script_dir}/../builder/env/musl-cross-make"
...
{
	printf '%s\n' "Build a static cross compiler and export it as compressed tarball)"
	printf 'Usage: %s [-p|--packages-dir <arg>] [-a|--arch <type string>] [--(no-)force-build-image] [-v|--(no-)verbose] [-h|--help] [<commit-id>]\n' "$0"
...
build_and_export_package()
{
    ...
    _docker_cmd="./build_and_export_cross-compiler.sh ${_arg_commit_id} ${_flag_verbose} -a ${_arg_arch}"

    (mkdir -p ${_arg_packages_dir} && \
        docker run --rm -ti \
            --mount type=bind,source="${script_dir}/../builder",target="/usr/local/src/musl-cross-maker/builder" \
            --mount type=bind,source="${_arg_packages_dir}",target="/usr/local/src/musl-cross-maker/packages" \
